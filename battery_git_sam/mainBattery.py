...
plt.xticks(rotation='vertical')
ax.bar(np.array(v0_analyticVin.index.values)[clusters[0]],(np.array(v0_analyticVin.values))[clusters[0]],color="g")
...
df = pd.DataFrame({"clusters":z.values,"count":y.values})
clst = df["clusters"].unique().tolist()
colorf = dict(zip(clst, colors))
ax = sns.boxplot(x="clusters",
                 y="count",
                 data=df)
for i in range(3):
    box = ax.artists[i]
    box.set_facecolor(colorf[clst[i]])
