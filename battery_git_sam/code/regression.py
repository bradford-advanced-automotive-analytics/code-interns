...
def regression(df,d0,perc_trainset,bycols,alpha=5e-4):
    df = df.sample(frac=1).reset_index(drop=True)
    n = int(dcopy.shape[0]*perc_trainset)
    train_data,test_data = df.loc[:n],df.loc[n:]
    #print(train_data.shape,test_data.shape)
    reg = linear_model.Ridge(alpha=alpha,fit_intercept=True,tol=1e-3)
...
d0 = deepcopy(dd)
d0.dataset = d0.dataset[d0.dataset["_cycle"]=="drive"]
d0.dataset = d0.dataset[d0.dataset['cellbalancing_hvbattbalancingstatus']=="noBalancing"]
...
analytic = list(analytic_subgroups.keys())[0]
...
tree = pm.ProfileTree(d0.dataset[cat])
prof = tree.getProfiles()
prof.sort()
prof_col = d0.dataset[cat].apply(lambda r : pm.find(r,prof,isSorted=True), axis=1)
d_cop = deepcopy(d0.dataset)

predicted_profiles = {}

for chosen_profile in range(len(prof)) :
# change a profile then apply a regression
    d0.dataset = deepcopy(d_cop)
    d0.dataset["profile"] = prof_col
    chosen_profile = np.random.randint(len(prof))
    d0.dataset = d0.dataset[d0.dataset["profile"]==chosen_profile]
    print("n rows =",d0.dataset.shape[0],prof[chosen_profile].events)
    if(d0.dataset.shape[0]>150):
        d0.select(selectCols)
        #categorical_dict = d0.splitSeveralToNumerous(cat) 
        #print(d0.dataset.columns,"DUMMY VARIABLES ENCODING")
        ...
        (groupsCycles,reg,n,score) = searchRate(dcopy,d0,["encryptedvin","_cycle","_num_cycle"],rate=0.07)
        ...
        accepted = input("Accept regression : ")
        while(accepted!="0" and accepted!="1"):
            accepted = input("Accept regression : ")
            print("0 or 1 expected")
        if(accepted):
            predicted_profiles[str(prof[chosen_profile].events)]=reg
        plt.close()